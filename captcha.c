/* captcha.c - captcha generator Version 0.1.0                       */
...
int main()
   ...
   /* the rucrc random number generator uses */
   /* the RANDU+CRC algorithm */
   /* plus the Bays-Durham shuffle */
   rufmt *ru;               /* rucrc structure */
   ru = (rufmt *) rucrcinit();    /* initialize the rucrc generator */
   /*******************************************/
   ...
   *p = '\0';        /* terminate the text string */
   ...
   picrowsz = 3 * ((txtlen*15) + 80 + (rucrcint(ru,40)));
   piccolsz = 24 + 40 + (rucrcint(ru,40));
   ...
   while (txtindx < txtlen)
	 ...
	 while (col < 12)
	    ...
	    if (*fntp == '*')
	       ...
	       *picp++ = rucrcpwr(ru,6);     /* 0-63 color depth */
	       *picp++ = rucrcpwr(ru,6);     /* 0-63 color depth */
	       *picp++ = rucrcpwr(ru,6);     /* 0-63 color depth */
	       } /* if asterisk in font */
	    col++;
