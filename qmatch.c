/* qmatch.c - FIFO Queue Match for rucrc  Version 0.1.0              */
...
/* The period length of rucrc approximates 1.2e+61036.     */
/* The period length of GSL taus2 is 3.09e+26.             */
/* The speed of rucrc is 1/6 faster than the GSL version   */
/* of taus2.                                               */

/* This program uses an array of 16384 32-bit integers    */
/* The period length of a 32-bit integer is 2^32-1.       */
/* The period length of 16384 32-bit integers is the      */
/* factorial of 16384, or 1.2e+61036.                     */
...
void enque(qfmt *tail, unsigned int outpt)
   ...
   newnode->prev = tail;
   /* The outpt parameter is the output of an rucrc random number */
   /* generator cycle */
   newnode->outpt = outpt;
   } /* enque */
...
void deqenq(qfmt *head, qfmt *tail, unsigned int outpt)
   ...
   /* rucrc random number generator cycle */
   node->outpt = outpt;
   } /* deqenq */
...
int main()
   ...
   unsigned int outpt;         /* 32-bit output from an rucrc cycle */
   ...
   while (1)         /* infinite loop */
	 ...
	 char str[128];
         move(5,24);
         sprintf(str,"rucrc with Bays-Durham Shuffle");
         addstr(str);
	 move(10,27);
